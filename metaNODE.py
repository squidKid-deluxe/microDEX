...
# ======================================================================
PATH = os.path.join(os.path.abspath(os.path.dirname(__file__)), "pipe")
...
def Bitshares_Trustless_Client():
    ...
    while True:
        try:
            with open(os.path.join(PATH, "metaNODE.txt"), "r") as handle:
...
def race_append(doc="", text=""):
    ...
    while True:
        ...
        try:
            if iteration > 10:
                break
            with open(os.path.join(PATH, doc), "a+") as handle:
                handle.write(text)
                handle.close()
                break
        except Exception as error:
...
def race_write(doc="", text=""):
    ...
    while True:
        ...
        iteration += 1
        try:
            with open(os.path.join(PATH, doc), "w+") as handle:
                handle.write(text)
                handle.close()
                break
        except Exception as error:
...
def race_read(doc=""):
    ...
    while True:
        ...
        iteration += 1
        try:
            with open(os.path.join(PATH, doc), "r") as handle:
                ret = handle.read().replace("'", '"')
                handle.close()
                try:
                    # ret = json_load(ret)
...
def watchdog():
    ...
    while True:
        try:
            try:
                with open(os.path.join(PATH, "watchdog.txt"), "r") as handle:
                    ret = handle.read()
                    handle.close()
                ret = json_load(ret)
                ...
                with open(os.path.join(PATH, "watchdog.txt"), "w+") as handle:
                    handle.write(msg)
                    handle.close()
                msg = str(latency)
    ...
    for item, asset_id in enumerate(asset_ids):
        for _, balance in enumerate(ret):
            if balance["asset_id"] == asset_id:
                balances[asset_id] += float(balance["amount"]) / 10 ** asset_precisions[item]
    return balances
...
def rpc_open_orders(rpc, cache):
    ...
    """
    ret = wss_query(rpc, ["database", "get_full_accounts", [[cache["account_name"]], False]])
    try:
...
def rpc_last(rpc, cache):
    ...
    """
    ticker = wss_query(rpc, ["database", "get_ticker", [cache["currency"], cache["asset"], False]])
    ...
    while True:
        ...
        try:
            ...
            for util in range(utilizations):
                ...
                if handshake_max == 5:
                    set_timing = "                  " + it("red", "RESOLVING MEAN NETWORK SPEED")
                ...
                # storage['bw_depth'] = max(int(len(nodes) / 6), 1)
                if (len(white) < storage["bw_depth"]) or (len(black) < storage["bw_depth"]):
                ...
                print(
                    "ping             ",
                    "%.2f %.1f %.1f %s %.2f" % (ping_latency, ping_max, ping_r, ping_b, ping_p),
                )
                print(
                    "mean ping        ",
                    (it("yellow", ("%.3f" % ping))),
...
def bifurcation(storage, cache):
    ...
    while True:
        try:
            ...
            currency_holding = float(precision(currency_holding, cache["currency_precision"]))
...
def main(storage=None, cache=None):
    """
    Primary event backbone
    """
    if storage is None and cache is None:
        storage, cache = spool()
